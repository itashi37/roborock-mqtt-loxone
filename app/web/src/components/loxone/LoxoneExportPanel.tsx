...
interface Props {
  ...
  mqttRobotCount: number;
  templateStatus: { native_generation: boolean; format_verified: boolean; reason: string; required_samples: string[] };
  perRobot: number;
  limit: number;
  required: number;
  exceeds: boolean;
...
export function LoxoneExportPanel({ robots, selection, mqttRobotCount, templateStatus, perRobot, limit, required, exceeds, exporting, onRobotSelected, onDownload }: Props) {
  return <section className="rounded-2xl border border-border bg-card p-5 shadow-sm" aria-labelledby="loxone-export-heading">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Select the robots, commandable rooms and scenes to include. The ZIP contains MQTT and/or Direct HTTP plans according to each robot’s enabled transports. No credentials are exported.</p>
        ...
        <div className={`mt-4 rounded-xl border p-4 ${exceeds ? 'border-amber-500/40 bg-amber-500/10' : 'border-border bg-background'}`}>
          ...
          <p className="mt-3 text-sm"><strong>Calculation:</strong> {mqttRobotCount} MQTT-enabled selected robot{mqttRobotCount === 1 ? '' : 's'} × {perRobot} subscriptions each (<code>/core</code> + <code>/activity</code>) = <strong>{required} subscriptions</strong>. Direct HTTP robots use no MQTT subscription.</p>
        ...
        <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4"><div className="flex items-center gap-2"><strong>Native Loxone Config templates</strong><StatusBadge tone="amber">Awaiting validated samples</StatusBadge></div><p className="mt-2 text-sm text-muted-foreground">{templateStatus.reason}. This download remains a safe import/configuration plan and contains no invented XML.</p><details className="mt-2 text-sm"><summary className="cursor-pointer">Exports needed from your exact Loxone Config version</summary><ul className="mt-2 list-disc space-y-1 pl-5">{templateStatus.required_samples.map(sample => <li key={sample}><code>{sample}</code></li>)}</ul></details></div>
      </div>
